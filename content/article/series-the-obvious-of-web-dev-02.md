...
**ストレージ・データベース** :  
ストレージはデータの長期記憶のためのシステムや装置のこと、データベースはデータを保管・管理・検索するためのシステムのことを指す。  
プログラムは大小や保存期間の長さを問わず様々なデータを扱う。システムに関わる限りは、こういったデータの保存領域に関わることは避けられないだろう。
...
DevOpsという言葉自体は、開発と運用といういわばシステムを作る部門と動かす部門とが分業を行うことよりも、統合されたプロセスによる迅速で確実なリリースを進める方が良い、という考え方や、それに付随するプラクティス・組織の構造などに対するアプローチの総称だ[^9]。第一のモダンの発展の時期よりも前に出てきた概念ではあるが、第一のモダンの発展により、理論から実現へと大きく歩を進めた。  
